...
const Int_t min1=36292;//Lowf(fwe);//cambiar aqui
const Int_t max1=36472;//highf(fwe,n);
...
void KGOQc(TString nombrefile1= "resonador_0.txt"){
...
///////--------------------------------------K
   TFile *GaoK = new TFile("ParametersGaoK.root","update","KGao");

   Double_t AM;
TTree *tree2 = new TTree("tree2","data"); 
tree2->Branch("frecuency",&x1,"frequency/F");  
tree2->Branch("Amplitude",&AM,"Amplitude/D"); 
tree2->Branch("zero",&q1,"zero/F"); 
...
///////--------------------------------------K  
comp2->SetPoint(nlines,x1,AM);
//comp2->SetPointError(i,zeroc.l;2,delta2); por si quieren agregar errores
///////--------------------------------------K
}
...
///////--------------------------------------K  
Long64_t N1 = tree2->Draw("frequency:Amplitude:zero:zero","", "goff");
TGraphErrors *treegraph1 = new TGraphErrors(N1,tree2->GetV1(),tree2->GetV2(),tree2->GetV3(),tree2->GetV4());
///////--------------------------------------K  
...
parameters[2]=  PI/12;  //theta cerrarlo
...
//cout<<"thank for use israel's programs";lol
...
CHI+=((S12[i]-func(frec[i],parameters2))*TComplex::Conjugate(S12[i]-func(frec[i],parameters2))).Re();
//cout<<"CHI:   "<<CHI<<endl;
...
///////--------------------------------------K
TTree *tree1 = new TTree("TreePar","Parameters");
TTree *tree3 = new TTree("tree3","tree3");

//TDirectory *main = GaoK->mkdir("100mK"); <------------------------------------------------------ ---------------------rememeber this!!!


//tree1->Branch("frpmax",&max1,"frpmax/D");
//tree1->Branch("frpmin",&min1,"frpmin/D");

cout << "posicion de resonancia: "<< f << "  //// pos max :  "<< max1<< "  //// pos min :  "<< min1 << endl ;

Double_t fpos = f;

tree1->Branch("frp",&fpos,"frp/D");
cout<< "f again pos:  " << fpos <<endl;
cout << "frecuencia de resonancia: "<< val<< "  Qr :  "<< val3<< "  Qc :  "<< val22<< endl ;

tree1->Branch("f0",&val,"f0/D");
tree1->Branch("Qr",&val3,"Qr/D");
tree1->Branch("Qc",&val22,"Qc/D");

Double_t diff, posa, posb, poss,AMValue,nop,nmax1,nmin1,fres; //ycomp2, delta2,dsum2,zeroc2;

tree2->Branch("diff",&diff,"diff/D");


tree2->Scan("Amplitude");
UInt_t Ent2 = tree2->GetEntries();
cout<<"entries tree2:  "<< Ent2<<endl;

//TGraph *graphC= new TGraph();

tree3->Branch("pos",&poss,"pos/D");
tree3->Branch("diff",&diff,"diff/D");
tree3->Branch("AMValue",&posb,"AMValue/D");  /////////////////-------------------------------> here 23 august
tree3->Branch("nop",&nop,"nop/D");
tree3->Branch("nmax1",&nmax1,"nmax1/D");
tree3->Branch("fres",&fres,"fres/D");
tree3->Branch("nmin1",&nmin1,"nmin1/D");

for (UInt_t i=0; i < Ent2; i++) {
//cout << "i //////" <<i << endl;
TLeaf *Fa = tree2->GetLeaf("Amplitude");
Fa->GetBranch()->GetEntry(1000);//<---------------------------------------- selection here 
posa = Fa->GetValue();

TLeaf *Fb = tree2->GetLeaf("Amplitude");
Fb->GetBranch()->GetEntry(i);
posb = Fb->GetValue();
...
if (diff>5) {
//tree2->Scan("diff");
cout <<" difference position //// "<< i << endl; 
poss = i;
cout <<" diff pos with i //// " << poss << endl; 

cout <<" difference //// "<< diff << endl; 
cout <<" amplitude value  //// "<< posb << endl; 
nop=fpos-poss;
nmax1=fpos+nop;
cout <<" number of points in fit //// "<< nop << endl; 
nmin1=poss;
cout <<" new max //// "<< nmax1 << endl; 
cout <<" new min  //// "<< poss<< endl; 
//Int_t n =2.;    //PARA LA CONSTANTE 
  Double_t r[n], t[n];  

  for (Int_t i=0;i<Ent2;i++) { //estas 
      r[i] = i;                   //lineas
      t[i] = posb;                  // agregan una
        } // constante
//graphC->SetPoint(n,r,t);
tree3->Fill();
...
// Para hacer la linea de la resta de 5 dB
TLeaf *Fit2y = tree3->GetLeaf("AMValue");
Fit2y->GetBranch()->GetEntry(0);
Double_t y3 = Fit2y->GetValue();
...
Double_t r[n], t[n];  
  ...
  TGraph *graphC= new TGraph(n,r,t);

//TLeaf *Fit2eH2 = tree2->GetLeaf("eyH2");
//Fit2eH2->GetBranch()->GetEntry(i);
//Double_t dyhigh2 = Fit2eH2->GetValue(); 
    
//TLeaf *Fit2eL2 = tree2->GetLeaf("eYL2");
//Fit2eL2->GetBranch()->GetEntry(i);
//Double_t dylow2 = Fit2eL2->GetValue();

//TLeaf *Fit2zero2 = tree2->GetLeaf("zero2");
//Fit2zero2->GetBranch()->GetEntry(i);
//zeroc2 = Fit2zero2->GetValue();
...
outfile2.close();
///////--------------------------------------K
TCanvas *c2 = new TCanvas("c2", "Frecuencia");

treegraph1->SetTitle(""); 

treegraph1->Draw("AP");
treegraph1->GetXaxis()->SetTitle("Frequency[GHz]");
treegraph1->GetYaxis()->SetTitle("Amplitude[dB]");
treegraph1->GetYaxis()->SetLabelSize(0.028);
treegraph1->GetYaxis()->SetTitleOffset(1.2);
treegraph1->GetXaxis()->SetTitleSize(0.1);
treegraph1->GetYaxis()->CenterTitle();
treegraph1->GetXaxis()->SetLabelSize(0.028);
treegraph1->GetXaxis()->SetTitleOffset(.8);
treegraph1->GetXaxis()->SetTitleSize(0.028);
