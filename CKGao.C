...
const Int_t min1=31637;//Lowf(fwe);//cambiar aqui
const Int_t max1=35267;//highf(fwe,n);
...
void CKGao(TString nombrefile1= "resonador_1.txt"){
...
auto oldfile = TFile::Open("./ParametersGaoK.root", "UPDATE");
   TTree* oldtree;
   
    oldfile->GetObject("TreePar", oldtree);
   oldtree->Scan("*");
   oldtree->SetBranchAddress("frp",&fpos);
   oldtree->SetBranchAddress("f0",&val);
   oldtree->SetBranchAddress("Qr",&val3);
   oldtree->SetBranchAddress("Qc",&val22);
 


oldtree->Fill();
cout<<"Scan Actualizado supuestamente"<<endl;
oldtree->Scan("*");
oldtree->Write();
